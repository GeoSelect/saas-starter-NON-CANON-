...
CREATE POLICY IF NOT EXISTS "Organization owners can update" ON public.organizations
  FOR UPDATE USING (owner_id = auth.uid());

-- Organization members policies
CREATE POLICY IF NOT EXISTS "Members can view their memberships" ON public.organization_members
  FOR SELECT USING (user_id = auth.uid());

-- Projects policies
CREATE POLICY IF NOT EXISTS "Users can view their projects" ON public.projects
  FOR SELECT USING (
    user_id = auth.uid() OR
    EXISTS (
      SELECT 1 FROM public.organization_members
...
CREATE POLICY IF NOT EXISTS "Users can create their own posts" ON public.posts
  FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Users can update their own posts" ON public.posts
  FOR UPDATE USING (user_id = auth.uid());
...
-- Files policies
CREATE POLICY IF NOT EXISTS "Users can view their files" ON public.files
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Users can upload files" ON public.files
  FOR INSERT WITH CHECK (user_id = auth.uid());
